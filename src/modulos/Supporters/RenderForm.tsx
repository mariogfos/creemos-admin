...
const RenderForm = ({
  ...
  return (
    ...
    >
      ...
      <Select label="Provincia" name="prov_code" value={formState.prov_code || ''} options={extraData?.provs || []} optionLabel="name" optionValue="code" onChange={handleChange} error={localErrors} />
      {getMuns().length > 0 && ( <Select label="Municipio" name="mun_code" value={formState.mun_code || ''} options={getMuns()} optionLabel="name" optionValue="code" onChange={handleChange} error={localErrors} /> )}
      {getDmuns().length > 0 && ( <Select label="Distrito municipal" name="dist_code" value={formState.dist_code || ''} options={getDmuns()} optionLabel="name" optionValue="code" onChange={handleChange} error={localErrors} /> )}
      {getLocals().length > 0 && ( <Select label="Localidad" name="local_code" value={formState.local_code || ''} options={getLocals()} optionLabel="name" optionValue="code" onChange={handleChange} error={localErrors} /> )}
      {getRecints().length > 0 && ( <Select label="Recinto" name="recint_code" value={formState.recint_code || ''} options={getRecints()} optionLabel="name" optionValue="code" onChange={handleChange} error={localErrors} /> )}
    </DataModal>
  );
};
